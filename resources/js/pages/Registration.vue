<template>
	<div class="row">
		<div class="col-lg-6 offset-lg-3">
			<div class="card px-3 pt-4 pb-2 mt-2 border-0">
				<h4 class="text-center font-weight-bold">Get your free account</h4>

				<div class="card-body">
					<form @submit.prevent="register">
						<div class="form-row">
							<div class="form-group col-lg-6">
								<label for="first_name" class="col-form-label col-form-label-sm label">First
								Name</label>
								<input type="text"
									id="first_name"
									...
									:style="form.errors.has('first_name') ? 'border-color: #e3342f' : 'border-color: #8D8C8A'">
								<p class="small text-danger mt-1" v-if="form.errors.has('first_name')">{{form.errors.get('first_name') }}</p>
							</div>
							<div class="form-group col-lg-6">
								...
								<input type="text"
									...
									:style="form.errors.has('last_name') ? 'border-color: #e3342f' : 'border-color: #8D8C8A'">
								<p class="small text-danger mt-1" v-if="form.errors.has('last_name')">{{ form.errors.get('last_name')}}</p>
						</div>
					</div>
					...
					<div class="form-row">
						<div class="form-group col-lg-6">
							...
							<input type="text"
								...
								:style="form.errors.has('email') ? 'border-color: #e3342f' : 'border-color: #8D8C8A'">
							<p class="small text-danger mt-1" v-if="form.errors.has('email')">{{ form.errors.get('email') }}</p>
						</div>
						<div class="form-group col-lg-6">
							<label for="email_confirmation" class="col-form-label col-form-label-sm label">Confirm E-mail</label>
							<input type="text"
				...
				<div class="form-row">
					<div class="form-group col-lg-6">
						...
						<input type="text"
							...
							:style="form.errors.has('birthday') ? 'border-color: #e3342f' : 'border-color: #8D8C8A'">
						<p class="small text-danger mt-1" v-if="form.errors.has('birthday')">{{ form.errors.get('birthday')}}</p>
				</div>
				<div class="form-group col-lg-6">
					...
					<select id="gender"
						...
						:style="form.errors.has('gender') ? 'border-color: #e3342f' : 'border-color: #8D8C8A'">
					<option value="male">Male</option>
					<option value="female">Female</option>
				</select>
				<p class="small text-danger mt-1" v-if="form.errors.has('gender')">{{ form.errors.get('gender') }}</p>
			</div>
		</div>

		<button class="btn submit-button">Register</button>
	</form>
</div>
</div>
</div>
...
<script>
	...
	export default {
		...
		methods: {
			async register() {
				try {
					const response = await this.form[api.auth.register.method](api.auth.register.url());
